...
printf("Enter Data: ");
 ...
 sendto(sockfd,
&frame_send, sizeof(Frame), 0, (struct
...
sockaddr*)&serverAddr, &addr_size);
 ...
 if( f_recv_size > 0 &&
frame_recv.sq_no == 0 && frame_recv.ack ==
frame_id+1){
 printf("[+]Ack Received\n");
 ack_recv = 1;
 }else{
...
sockaddr*)&newAddr, &addr_size);
 if (f_recv_size > 0 &&
frame_recv.frame_kind == 1 && frame_recv.sq_no
== frame_id){
...
frame_recv.sq_no + 1;
 sendto(sockfd,
&frame_send, sizeof(frame_send), 0, (struct
sockaddr*)&newAddr, addr_size);
 printf("[+]Ack Send\n");
 }else{
 printf("[+]Frame Not
...
// Server
// $./s 5080
// [+]Frame Received: hello i am manu
// [+]Ack Send
// [+]Frame Received: hi
// [+]Ack Send
// [+]Frame Received: j
